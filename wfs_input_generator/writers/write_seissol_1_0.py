...
DEFAULT_CONFIGURATION = {
    ...
    # What is the unit, meaning of material reference values?? kg/m^3. rho mü=Pascal lambda=Pascal
...
def _check_configuration(config):
    ...
    if config.minspace_order is not config.maxspace_order:
        ...
        if str(1) in config.zone_order_flag:
            ...
            except: IOError, 'Zone order flag = 1, but \
...
def _write_used_stations(used_stations):
    ...
    try:
        used_stations_json = json.dumps(used_stations, sort_keys=True, indent=4, \
        separators=(',', ': '))
...
class CheckStationsMesh(object):
    ...
    def __init__(self, filename_mesh='/home/msimon/svn/repos/verce/All/JRA/JRA1/python/test_ressources/inputfiles/eucrust_small_new.neu', \
    station_coordinates=str()):
    ...
    def __read_mesh(self, filename_mesh = '/home/msimon/svn/repos/verce/All/JRA/JRA1/python/test_ressources/inputfiles/eucrust_small_new.neu'):
        ...
        if 3 == NDFCD:
            ...
            for boundaries in range(NBSETS):
                ...
                s.isdigit()]
                nelem = first_line[2]
                boundary_elements[str(first_line[0])] = np.fromfile(file \
                = f, count = nelem*3, sep = " ", dtype=int).reshape(3, \
                nelem)[0]
                position = f.tell()
        else:
    ...
    def move_points_to_mesh(self, steplength=1.01):
        ...
        flatten()).reshape(len(np.compress(inv_in3d.flatten(), \
